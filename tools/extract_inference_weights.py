"""
从完整训练 checkpoint 中提取推理专用权重，并预计算 cell_size_code。

输出文件：
  downloads/inference_net.pth   — RGBEnc + RGBDec 权重
  downloads/cell_size_code.pt   — 预计算的 cell size 编码张量

用法：
  source ~/py39/bin/activate
  python tools/extract_inference_weights.py
"""
...
from models.basic_layer import MLP

PROJECT_ROOT = Path(__file__).parent.parent
DOWNLOADS = PROJECT_ROOT / "downloads"

# 与 inference.py 中完全一致的 MLP_code 常量
MLP_code = [
...
def strip_module_prefix(state_dict):
  """移除 DataParallel 训练产生的 module. 前缀"""
...
def main():
  # 1. 加载完整 C2PGen checkpoint
  ga_path = DOWNLOADS / "160_net_G_A.pth"
  print(f"Loading {ga_path} ...")
  full_state = torch.load(str(ga_path), map_location='cpu')
  full_state = strip_module_prefix(full_state)

  # 统计原始 key
  all_keys = set(full_state.keys())
  print(f"Total keys in checkpoint: {len(all_keys)}")

  # 2. 提取 RGBEnc + RGBDec 权重
  inference_keys = {k for k in all_keys if k.startswith('RGBEnc.') or k.startswith('RGBDec.')}
  inference_state = {k: v for k, v in full_state.items() if k in inference_keys}
  print(f"Inference keys (RGBEnc + RGBDec): {len(inference_keys)}")

  out_path = DOWNLOADS / "inference_net.pth"
  torch.save(inference_state, str(out_path))
  size_mb = out_path.stat().st_size / (1024 * 1024)
  print(f"Saved {out_path} ({size_mb:.1f} MB)")

  # 3. 提取 MLP 权重，预计算 cell_size_code
  mlp_keys = {k for k in all_keys if k.startswith('MLP.')}
  mlp_state = {k.replace('MLP.', ''): v for k, v in full_state.items() if k in mlp_keys}
  print(f"MLP keys: {len(mlp_keys)}")

  # 构造 MLP 并加载权重（参数与 C2PGen.__init__ 一致）
  mlp = MLP(256, 2048, 256, 3, norm='none', activ='relu')
  mlp.load_state_dict(mlp_state)
  mlp.eval()

  with torch.no_grad():
    code = torch.tensor(MLP_code).reshape(1, 256, 1, 1)
    cell_size_code = mlp(code)

  code_path = DOWNLOADS / "cell_size_code.pt"
  torch.save(cell_size_code.cpu(), str(code_path))
  print(f"Saved {code_path}, shape={cell_size_code.shape}")

  # 4. 汇总
  dropped_keys = all_keys - inference_keys - mlp_keys
  print(f"\nDropped keys (PBEnc etc.): {len(dropped_keys)}")
  print("Done.")
