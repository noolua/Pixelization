...
Tries multiple strategies in order:
  ...
  5. onnxconverter_common + block all FP16-sensitive ops (Pow/Sqrt/ReduceSum/etc)
  6. Manual weight-only FP16 (initializers → FP16, Cast → FP32 for each use)

Strategy 6 is a fallback: model runs FP32 ops but weights are stored as FP16,
...
def print_stats(path):
  ...
  fp16_inits = sum(1 for init in model.graph.initializer
                   if init.data_type == TensorProto.FLOAT16)
  fp32_inits = sum(1 for init in model.graph.initializer
                   if init.data_type == TensorProto.FLOAT)
  print(f"    Size: {size_mb:.1f} MB, Nodes: {len(model.graph.node)}, "
        f"Cast nodes: {cast_count}")
  print(f"    Initializers: {fp16_inits} FP16, {fp32_inits} FP32")
  print(f"    Top ops: {op_counts.most_common(8)}")
...
def convert_manual(model_path, output_path):
  ...
  print(f"    Converted {converted} initializers to FP16, "
        f"{skipped} kept in FP32 (overflow)")
...
def convert(args):
  ...
  for init in model.graph.initializer:
    if init.data_type == TensorProto.FLOAT:
      ...
      if np.any(np.abs(arr) > 65504):
        large_values += 1
      if np.any((np.abs(arr) > 0) & (np.abs(arr) < 6e-8)):
        tiny_values += 1
  if large_values:
    print(f"  WARNING: {large_values} initializers have values > FP16 max (65504)")
  if tiny_values:
    print(f"  WARNING: {tiny_values} initializers have values < FP16 min positive (~6e-8)")
  ...
  strategies = [
      ...
      ("block all FP16-sensitive", {
          "op_block_list": {
              "InstanceNormalization", "Resize",
              "Pow", "Sqrt", "ReduceSum",
              "ReduceMean", "Reciprocal",
          },
          "disable_shape_infer": True,
          "check_fp16_ready": False,
      }),
  ...
  # Strategy 6: manual weight-only (always works, size reduction only)
