...
- [ ] 布局重构
  - [ ] 管线编排区：纵向卡片流（移动端友好），每张卡片宽度 100%
  - [ ] 卡片展开/折叠动画（CSS transition max-height）
  - [ ] 卡片之间的管道连接线（竖线 + 箭头，CSS 伪元素实现）
  - [ ] 拖拽时的视觉反馈（虚线边框、阴影提升）
- [ ] 状态指示
  - [ ] 未执行：灰色边框
  - [ ] 执行中：蓝色边框 + 旋转图标
  - [ ] 已完成：绿色边框 + 缩略图
  - [ ] 执行失败：红色边框 + 错误信息
  - [ ] 需要重新执行（参数已变更）：橙色边框 + 提示文案
- [ ] 响应式适配
  - [ ] 桌面端（≥768px）：管线区最大宽度 800px 居中
  - [ ] 移动端（<768px）：全宽，参数面板堆叠
- [ ] 上传区保留不变
  - [ ] 上传区与管线区分离，上传后管线区才出现
  - [ ] 切换图片时清除所有 blob，保留管线结构
- [ ] 清理遗留代码
  - [ ] 删除硬编码的 image-box DOM
  - [ ] 删除全局 blob 变量（selectedFile 除外）
  - [ ] 删除旧的事件绑定代码
