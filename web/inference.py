...
class Model():
  ...
  def load(self):
    with torch.no_grad():
      ...
      ga_path = PROJECT_ROOT / "downloads" / "160_net_G_A.pth"
      G_A_state = torch.load(str(ga_path), map_location='cpu')
      ...
      alias_path = PROJECT_ROOT / "downloads" / "alias_net.pth"
      alias_state = torch.load(str(alias_path), map_location='cpu')
